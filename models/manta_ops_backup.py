...
class MDNAFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, Q, K, V, nb_idx, nb_valid, scale):
        ext = _get_ext()
        Out, Lse = ext.mdna_forward(Q, K, V, nb_idx, nb_valid, scale)
        ctx.save_for_backward(Q, K, V, Out, Lse, nb_idx, nb_valid)
        ctx.scale = scale
        return Out

    @staticmethod
    def backward(ctx, dO):
        Q, K, V, Out, Lse, nb_idx, nb_valid = ctx.saved_tensors
        ext = _get_ext()
        dQ, dK, dV = ext.mdna_backward(
            Q, K, V, Out, Lse, dO.contiguous(), nb_idx, nb_valid, ctx.scale
        )
        return dQ, dK, dV, None, None, None


def mdna_attention(Q, K, V, nb_idx, nb_valid, scale):
    """Sparse 1D multi-scale dilated neighborhood self-attention via CUDA."""
    return MDNAFunction.apply(Q, K, V, nb_idx, nb_valid, scale)
...
def tanca_attention(Q, K, V, nb_idx, nb_valid, scale):
    """Sparse cross-attention with O(n_vars) memory via CUDA."""
    return TANCAFunction.apply(Q, K, V, nb_idx, nb_valid, scale)
