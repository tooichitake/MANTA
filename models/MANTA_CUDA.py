...
def build_reverse_na_indices(nb_idx, nb_valid, N, max_K):
    """Build reverse neighbor index: for each key position m, which queries attend to it.

    Returns rev_nb_idx [H, N, max_rev_K], rev_nb_valid [H, N, max_rev_K].
    """
    H = nb_idx.shape[0]
    # Count reverse neighbors for each (h, m)
    rev_counts = torch.zeros(H, N, dtype=torch.int32)
    for h in range(H):
        for n in range(N):
            for ki in range(max_K):
                if nb_valid[h, n, ki]:
                    m = nb_idx[h, n, ki].item()
                    rev_counts[h, m] += 1

    max_rev_K = int(rev_counts.max().item())
    rev_nb_idx = torch.zeros(H, N, max_rev_K, dtype=torch.int32)
    rev_nb_valid = torch.zeros(H, N, max_rev_K, dtype=torch.int8)

    # Fill reverse indices
    fill_pos = torch.zeros(H, N, dtype=torch.int32)
    for h in range(H):
        for n in range(N):
            for ki in range(max_K):
                if nb_valid[h, n, ki]:
                    m = nb_idx[h, n, ki].item()
                    pos = fill_pos[h, m].item()
                    rev_nb_idx[h, m, pos] = n
                    rev_nb_valid[h, m, pos] = 1
                    fill_pos[h, m] += 1

    return rev_nb_idx, rev_nb_valid


def build_na_buffers(n_heads, patch_num):
    """Build nb_idx, nb_valid, rev_nb_idx, rev_nb_valid for CUDA kernels."""
    ...
    rev_nb_idx, rev_nb_valid = build_reverse_na_indices(
        nb_idx_all, nb_valid_all, patch_num, max_K)
    return nb_idx_all, nb_valid_all, rev_nb_idx, rev_nb_valid, head_cfgs
...
class MDNA_CUDA(nn.Module):
    ...
    def __init__(self, n_heads, patch_num, d_model):
        ...
        nb_idx, nb_valid, rev_nb_idx, rev_nb_valid, self.head_configs = build_na_buffers(n_heads, patch_num)
        self.register_buffer('_nb_idx', nb_idx)
        self.register_buffer('_nb_valid', nb_valid)
        self.register_buffer('_rev_nb_idx', rev_nb_idx)
        self.register_buffer('_rev_nb_valid', rev_nb_valid)
    ...
    def forward(self, x):
        ...
        out = mdna_attention(q, k, v, self._nb_idx, self._nb_valid, self._rev_nb_idx, self._rev_nb_valid, scale)
...
class TANCA_CUDA(nn.Module):
    ...
    def forward(self, x_q, x_kv):
        ...
        out = tanca_attention(q, k, v, self._nb_idx, self._nb_valid, self._rev_nb_idx, self._rev_nb_valid, scale)
...
class Model(nn.Module):
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        """MS/S mode: endo/exo split (last var excluded from KV) + VarKVDrop."""
        if self.use_revin:
        ...
        endo = all_embed[:, -1:, :, :]       # Q: last variable only
        exo = all_embed[:, :-1, :, :]         # KV: all other variables

        # VarKVDrop on exo variables during training
        if self.training and exo.shape[1] > self.k_sample:
            idx = torch.randperm(exo.shape[1], device=exo.device)[:self.k_sample]
            idx = idx.sort().values
            exo = exo[:, idx]
    ...
    def forecast_multi(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        """M mode: all vars as Q+KV, VarKVDrop during training.
